...
class UdopTokenizer(T5Tokenizer):
        ...
        **kwargs
        ...
        if extra_ids > 0 and "<extra_id_0>" not in additional_special_tokens:
            ...
            additional_special_tokens.extend(
                [f"<extra_l_id_{i}>" for i in range(extra_ids)]
            )
            additional_special_tokens.extend(
                [f"</extra_l_id_{i}>" for i in range(extra_ids)]
            )
            additional_special_tokens.extend(
                [f"<extra_t_id_{i}>" for i in range(extra_ids)]
            )
            additional_special_tokens.extend(
                [f"</extra_t_id_{i}>" for i in range(extra_ids)]
            )
        ...
        if loc_extra_ids > 0 and "<loc_0>" not in additional_special_tokens:
            additional_special_tokens.extend([f"<loc_{i}>" for i in range(loc_extra_ids)])

        if other_extra_ids > 0 and "<other_0>" not in additional_special_tokens:
            additional_special_tokens.extend(
                [f"<other_{i}>" for i in range(other_extra_ids)]
            )
    ...
    def _convert_id_to_token(self, index):
        ...
        if index < self.sp_model.get_piece_size():
        ...
        elif index > self.sp_model.get_piece_size() + self._extra_ids * 5 + self._loc_extra_ids - 1:
            ...
            token = f"</extra_t_id_{self.vocab_size - self._other_extra_ids - self._loc_extra_ids - 1 - index}>"
        elif index > self.sp_model.get_piece_size() + self._extra_ids * 3 - 1:
            token = f"<extra_t_id_{self.vocab_size - self._other_extra_ids - self._loc_extra_ids - self._extra_ids - 1 - index}>"
        elif index > self.sp_model.get_piece_size() + self._extra_ids * 2 - 1:
            token = f"</extra_l_id_{self.vocab_size - self._other_extra_ids - self._loc_extra_ids - self._extra_ids * 2 - 1 - index}>"
        elif index > self.sp_model.get_piece_size() + self._extra_ids - 1:
            token = f"<extra_l_id_{self.vocab_size - self._other_extra_ids - self._loc_extra_ids - self._extra_ids * 3 - 1 - index}>"
        elif index > self.sp_model.get_piece_size() - 1:
            token = f"<extra_id_{self.vocab_size - self._other_extra_ids - self._loc_extra_ids - self._extra_ids * 4 - 1 - index}>"
        else:
            raise
        return token
...
class UdopConverter(SpmConverter):
    def vocab(self, proto):
        ...
        vocab += [(f"<extra_l_id_{i}>", 0.0) for i in range(num_extra_ids - 1, -1, -1)]
        vocab += [
            (f"</extra_l_id_{i}>", 0.0) for i in range(num_extra_ids - 1, -1, -1)
        ]
        vocab += [(f"<extra_t_id_{i}>", 0.0) for i in range(num_extra_ids - 1, -1, -1)]
        vocab += [
            (f"</extra_t_id_{i}>", 0.0) for i in range(num_extra_ids - 1, -1, -1)
        ]

        num_loc_extra_ids = self.original_tokenizer._loc_extra_ids
        vocab += [(f"<loc_{i}>", 0.0) for i in range(num_loc_extra_ids - 1, -1, -1)]

        num_other_extra_ids = self.original_tokenizer._other_extra_ids
        vocab += [
            (f"<other_0{i}>", 0.0) for i in range(num_other_extra_ids - 1, -1, -1)
        ]
