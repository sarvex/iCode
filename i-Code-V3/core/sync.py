...
class barrier_lock(object):
    def __init__(self, n):
        ...
        self.lock_shmname = f'barrier_lock_{id}'
...
class nodewise_sync_global(object):
    ...
    def __init__(self):
        ...
        id = int(random.random()*10000) + int(time.time())*10000
        self.id_shmname = f'nodewise_sync_id_shm_{id}'
...
class nodewise_sync(object):
    ...
    def broadcast_r0(self, data=None):
        ...
        if self.local_rank == 0:
            ...
            load_info_shm.buf[:datan] = data[:datan]
            self.barrier()
            self.barrier()
            load_info_shm.close()
            ...
            assert data is None, 'Rank other than 1 should input None as data!'
            self.barrier()
